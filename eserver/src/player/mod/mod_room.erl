...
start_friend_delay_quit_timer(Time,Fun,Player) ->
  gen_mod:cancel_absolute_timer(?MODULE,Time),
  gen_mod:start_absolute_timer(?MODULE, Time, ?QUIT_FRIEN_GAME(Fun,Player)).
...
handle_timeout(?DELAY_CALC_EVENT(Fun), #player{} = Player) ->

  Player2 = Fun(Player),
  %?DEBUG(?_U("收到延迟累消息:~p"), [Player2]),
  {ok, Player2};

handle_timeout(?QUIT_FRIEN_GAME(Fun,Player), #player{} = PlayerNew) ->

  Player2 = Fun(Player),
  ?DEBUG(?_U("收到延迟累quit消息:~p"), [Player2]),
  {ok, PlayerNew};
...
handle_s2s_cast({?Reset_GAME, Code1,Code2}, Player) ->
  ...
  {ok, Player2};
handle_s2s_cast({?GiveCard,RoomCardNum}, Player) ->


  Player2 = Player#player{card_room = RoomCardNum},
...
handle_s2s_cast({?Win_Video,SendId}, Player) ->
  ...
  {ok, Player2};
handle_s2s_cast({?FRIEND_GAME_OVER,_K1,_K2,_K3,_K4,_K5}, Player) ->
  ...
  {ok, Player2};
handle_s2s_cast({?START_GAME, Turn,MinMoney,CardOne,CardTWO,GameNum,CardNUM,TableMoney,IsFreshCard,LeftTime,_FriendDleteMoney}, Player) ->

  Code1 = wg_util:rand(1000,10000000),
  Code2 = wg_util:rand(1000,10000000),
  #player{id = SendId,money = Money,roomtype = RoomType,card_room = RoomCardNum} = Player,
  %?DEBUG(?_U("游戏开始玩家:~w 抓牌:下底数~w"), [Turn, MinMoney]),
  Msg = #gc_player_turn
  {
    usr_id  = Code1,
  ...
  ok = ?S2C_SEND_MERGE:player(SendId, MsgId, Msg),
  LeftCard= RoomCardNum - _FriendDleteMoney,
  ?IF(LeftCard < 0 , NewLeftCard = 0, NewLeftCard = LeftCard),

  case RoomType of
    ?Friend_Room_create->
      Player2 = Player#player{user_id = Code1,unit_id = Code2,gamenum = GameNum,card_room = NewLeftCard};
    ?Friend_Room_join->
...
handle_c2s(#cg_join_room{usr_id = BeginCode,room_type = RoomType,room_key = RoomKey,card_num = CardNum,game_time = GameTime,min_money = MinMoney,max_bet = MaxBet,is_double = IsDouble,unit_id = EndCode}, _MsgId,  #player{} = Player) ->
  #player{user_id = UserId, unit_id = UnitId,robot = Robot} = Player,
        ...
        {true,_} ->
          case RoomType of
            ...
            ?Friend_Room_create->
              ...
              Player2 = do_create_friend_room(NewPlayer,MinMoney,MaxBet,IsDouble,CardNum,GameTime);
            ?Friend_Room_join->
              NewPlayer = Player#player{roomtype = RoomType},
...
handle_c2s(#cg_deleteroom{}, _MsgId,  #player{} = Player) ->

  Player2 = delete_friend_room(Player),

  {ok, Player2};

handle_c2s(#cg_give_room_card{usr_id = BeginCode,giveplayerid = GiveId,givecardnum = GiveNum,unit_id = EndCode}, _MsgId,  #player{} = Player) ->

  #player{user_id = UserId, unit_id = UnitId,card_room = SenderCardNUM,id = SendId} = Player,
  Player2 =
    case BeginCode == UserId andalso EndCode == UnitId of
        true ->
          case SenderCardNUM >= GiveNum of
            true->
              case player_mgr:get_by_id(GiveId) of
                #player{card_room = OtherCardNum} = _Player ->
                  Req = {?GiveCard,OtherCardNum + GiveNum},
                  ok = player_server:s2s_cast(GiveId, ?MODULE, Req),
                  Code1 = wg_util:rand(1000,10000000),
                  Code2 = wg_util:rand(1000,10000000),
                  Msg = #gc_give_room_card
                  {
                    usr_id  = Code1,
                    senderid = SendId,
                    giveplayerid = GiveId,
                    sendcardnum = SenderCardNUM - GiveNum,
                    givecardnum = OtherCardNum + GiveNum,
                    unit_id = Code2
                  },
                  MsgId = ?PROTO_CONVERT({?MODULE, gc_give_room_card}),
                  ok = ?S2C_SEND_MERGE:player(SendId, MsgId, Msg),
                  ok = ?S2C_SEND_MERGE:player(GiveId, MsgId, Msg),
                  Player#player{card_room = SenderCardNUM - GiveNum,user_id = Code1,unit_id = Code2};
                _ ->
                case catch db_player:load(GiveId) of
                  #player{card_room = MyCardNum} = _Player ->
                    db_player:set_room_card(GiveId,MyCardNum + GiveNum),
                    Code1 = wg_util:rand(1000,10000000),
                    Code2 = wg_util:rand(1000,10000000),
                    Msg = #gc_give_room_card
                    {
  ...
  case IsRobot of
    true->
      ...
      Player2 = case Money =:= 0 of
                  true->
                    ?IF(IsMyTurn =:= true, do_quit_bet(Player),send_error_msg(Player));
                  false->
                    ?IF(IsMyTurn =:= true, do_money_bet(Player,Money,MsgBet),send_error_msg(Player))
                end;
    _->
      Player2 =case BeginCode == UserId andalso EndCode == UnitId of
                  true ->
                    %?INFO(?_U("玩家开始下注:~p"), [1]),
...
set_member_status(RoomType,PlayerId,RoomId,ChangIndex,PlayerState)->
  case RoomType of
    ...
    ?Friend_Room_join ->
      room_friend_mgr:set_member_status(PlayerId,RoomId,ChangIndex,PlayerState);
    ?KenFriend_Room_create ->
      ken_room_mgr:set_member_status(PlayerId,RoomId,ChangIndex,PlayerState);
    ?KenFriend_Room_join ->
      ken_room_mgr:set_member_status(PlayerId,RoomId,ChangIndex,PlayerState);
    _ ->
      ?INFO(?_U("更新全部room,信息失败:~p"), [RoomType]),
...
do_create_friend_room(Player,MinMoney,MaxBet,IsDouble,CardNum,GameTime)->
  ...
  case catch room_friend_mgr:create(Member,MinMoney,MaxBet,IsDouble,CardNum,GameTime) of
    #room{} = Room ->
...
do_friend_join(RoomId,Player)->
  ...
  case catch room_friend_mgr:add_member(RoomId,Member) of
    {#room{} = Room2, #room_member{}=Member2} ->
      ...
      Code1 = wg_util:rand(1000,10000000),
      Code2 = wg_util:rand(1000,10000000),
      #room{id = RoomId,need_card = CardNum,game_time = GameTime,min_money = MinMoney,max_bet = MaxBet,isDouble = IsDouble,cardone = CardOne,cardtwo = CardTwo,pork_list = PorkerList,member = NewMember,roomstate = RoomState,table_money = TableMoney,turn = CurTurn} = Room2,
...
do_add_friend_reconnect_room_member1(RoomType,RoomId,Member,Player)->
  case catch add_reconnect_member(RoomType,RoomId,Member) of
    {#room{} = Room2, #room_member{}=Member2} ->
      ...
      Code1 = wg_util:rand(1000,10000000),
      Code2 = wg_util:rand(1000,10000000),
      #room{need_card = CardNum,game_time = GameTime,min_money = MinMoney,max_bet = MaxBet,isDouble = IsDouble,id = RoomId,cardone = CardOne,cardtwo = CardTwo,pork_list = PorkerList,member = NewMember,roomstate = RoomState,table_money = TableMoney,turn = CurTurn,game_start_time = StartTime} = Room2,
      #room_member{id = SelfId,pos = MyPos,moneyRank = MoneyRankData,state = SelfState,score = SelfMoney} = Member2,
      #money_rank_data{name = SelfName,image = SelfHead} = MoneyRankData,

      case CurTurn =:= MyPos of
        true->
          Room3 = Room2#room{turn_begin_time = wg_util:now_sec()},
          set_room(Room3,RoomType);
        _->
          ok
      end,

      %?INFO(?_U("加入房间成功list:~p"), [ALLMemberList1]),
      Val = lists:map
      (
        fun(TempMember) ->
          %?INFO(?_U("房间成员:~p"), [TempMember]),
      ...
      [K1,K2,K3,K4,K5,_K6] = wg_lists:unzip_more(Val),
      ...
      (
        fun(Member3) ->
          ...
          #room_member{id = IdToSend} = Member3,
          ?IF( IdToSend=:= SelfId, ok = ?S2C_SEND_MERGE:player(IdToSend, MsgId, Msg),
            ok = ?S2C_SEND_MERGE:player(IdToSend, MsgId2, AddMsg))
      ...
      case is_friend_over(RoomType,RoomId) of
        ...
        false->
          %?INFO(?_U("check----:~p"), [1]),
          case TableMoney =:= 0 andalso StartTime =/= -1 of
            true->
              send_delay_msg(TableMoney);
            _->
              ?IF(length(NewMember) =:= 1, check_game_start2(RoomId,RoomType),ok)
          end
      end,
...
do_money_bet(#player{roomtype = RoomType} = Player,BetMoney,MsgBet)->
  case RoomType of
    ?Friend_Room_create ->
          ...
          case GameRes of
            ...
            ?Game_Lose_Double->
              ...
              case IsDouble of
                ...
                false->
                  Player2 = do_friend_game_losed(Player,Room,BetMoney,ResCard,?Game_Lose,NewPokerList,MsgBet)
              end,
              Player2;
            ?Game_Win->
...
change_friend_off_line_state(RoomType,PlayerId,RoomId)->
  case catch set_member_status(RoomType,PlayerId,RoomId,?CHANGE_STATE,?PLAYER_OFFlINE) of
    _Error ->
...
change_friend_join_ready_player_state(RoomType,PlayerId,RoomId,PlayerState,ChangIndex)->
  case catch set_member_status(RoomType,PlayerId,RoomId,ChangIndex,PlayerState) of
    NewMemberList when is_list(NewMemberList)->
      case catch get_room(RoomType,RoomId) of
        #room{game_start_time = GameStartTime}=_Room ->
          ?IF(GameStartTime =:= -1,ok,check_game_start(RoomId,PlayerId,NewMemberList,RoomType));
        _->
...
check_money_limit(RoomType,Player)->
  #player{money = Money,card_room = RoomCardNum} = Player,
  case RoomType of
    ?New_Room ->
...
do_friend_game_win(Player,Room,BetMoney,ResCard,GameRes,NewPokerList,MsgBet)->
  ...
  Room2 = Room#room{table_money = TableLeftMoney,pork_list = NewPokerList,member = NewMemberList,turn_begin_time = wg_util:now_sec()},
  set_room(Room2,RoomType),
...
do_game_double_losed(Player,Room,BetMoney,ResCard,GameRes,NewPokerList,MsgBet)->
  #player{id = PlayerId,room_id = RoomId,roomtype = RoomType,money = MyMoney,moneyRank = MoneyRank,loseRank = LoseRank,winRank = WinRank} = Player,
  #room{table_money = TableMoney,member = PlayerList,min_money = MinMoney2} = Room,
  [#room_member{pos = MyPos}=MyMember] = [X|| #room_member{id = MemberId} = X <-PlayerList,MemberId =:= PlayerId],

  TempMoney = MyMoney - BetMoney*2,
  ?IF(TempMoney >= 0,NewMoney = TempMoney,NewMoney = 0),
  ?IF(TempMoney >= 0,AddMoney = BetMoney*2,AddMoney = trunc(MyMoney/MinMoney2)*MinMoney2),

  BetMoney1 = AddMoney - BetMoney,
  ?IF(BetMoney1 >= 0,BetMoney2 = BetMoney1,BetMoney2 = 0),
  ...
  do_send_lose_double_game_result(PlayerList,Code1,MyPos,ResCard,GameRes,NewRank,NewWinRank,NewLoseRank,AfterGiveMoney,BetMoney,TableLeftMoney,Code2,MsgBet,BetMoney2),
  send_delay_msg(TableLeftMoney),
    ...
    true ->
      ...
      case ValidePlayerNum > 0 of
        true ->
          ...
          Req = {?START_GAME, TurnPos, MinMoney,CardOne,CardTwo,GameNum+1,length(NewPorkList),OldTabelMoney+MinMoney*PersonNum,PokerCardNum  < ?POKER_NUM_MAX,0,0},
...
do_friend_game_start(RoomId,Room,RoomType)->
  ...
  #room{id = Id,gameNum = GameNum,game_time = GameMaxTime,turn = OldTurn,need_card = NeedCard,member = MemberList,min_money = MinMoney,pork_list = PorkList,table_money = OldTabelMoney,game_start_time = GameStartTime,is_delete_roomcard = IsDeleteRoomCard} = Room,
  case Id =:= RoomId of
    true ->
      PersonNum = length(MemberList),
      PokerCardNum = length(PorkList),
...
reset_room_ready(Room,RoomType)->
  ...
  (
    fun(TempMember) ->
      #room_member{state = CurState} = TempMember,
      Member2 = case RoomType of
                  ?Friend_Room_create ->
                    ?IF(CurState =:= ?PLAYER_OFFlINE, NewState = ?PLAYER_OFFlINE,NewState = ?PLAYER_STATE_NORMAL),

                    TempMember#room_member{state = NewState};
                  ?Friend_Room_join ->
                    ?IF(CurState =:= ?PLAYER_OFFlINE, NewState = ?PLAYER_OFFlINE,NewState = ?PLAYER_STATE_NORMAL),

                    TempMember#room_member{state = NewState};
                  _->
                  TempMember#room_member{state = ?PLAYER_STATE_NORMAL}
                end,
...
do_send_lose_double_game_result(NewMemberList,Use_Id,Pos,CardRes,Result,RankMoney,RankWin_Money,RankLose,Curmoney,BetMoney,TabelMoney,Unit_id,MsgBet,DoubleNum) ->
  #cg_bet_money{one_chip_num = One,two_chip_num = Two,three_chip_num = Three,four_chip_num = Four,five_chip_num = Five}= MsgBet,
  Msg = #gc_bet_money
  {
    usr_id = Use_Id,
...
is_friend_over(RoomType,RoomId)->
  ...
  case RoomType =:= ?Friend_Room_create orelse RoomType =:= ?Friend_Room_join of
    true ->
      case catch get_room(RoomType,RoomId) of
        #room{game_over = GameOver,max_bet = MaxBet,table_money = TableMoney}=_Room ->
          case GameOver of
            true->
...
friend_quit_delay_quit_bet(Player)->
  Fun = fun(Player1) ->
    do_quit_bet(Player1)
        end,
  start_friend_delay_quit_timer(?FriendRoomDelayQuit,Fun,Player).
...
%%          end;
    ...
    #room_ken{}=Room ->
      do_quit_logic_friend_ken(Room,Player,QuitType);
    _->
      Msg = #gc_quit_room
      {
        usr_id                      = 6780,
...
do_quit_logic1(Room,Player,QuitType)->
  ...
  case Id =/= ?NULL_ROOMID of
    ...
    _->
      ?INFO(?_U("退出房间时,roomid等于null:~p,"), [Player]),
      Player#player{room_id = -1,roomtype = -1}
  end.

do_quit_logic_friend(Room,Player,QuitType)->
  #player{roomtype =  RoomType,id =PlayerId} = Player,

  #room{id = Id,member = PlayerList,game_start_time = GameStateTime,game_over = GameOver} = Room,
    ...
    true ->
      ...
      case length(TempList) > 0 of
        true->
          ...
          Player3 = case QuitType of
                    ?QUIT_NET_ERROR ->
                      %?IF(IsNotGameStart =:= true,Player#player{room_id = -1,roomtype = -1},Player#player{gamenum = Score});
                      change_friend_off_line_state(RoomType,PlayerId,Id),
                      Player#player{gamenum = Score};
                    ?QUIT_BY_SELF->
                      case GameStateTime =:= -1 of
                        true->
          ...
          MsgId = get_msg_id(gc_quit_room),
          ?IF(GameOver =:= true,?S2C_SEND_MERGE:player(PlayerId, MsgId, Msg),send_msg_broadcast(PlayerList,MsgId, Msg)),

          %IsNotGameStart = (GameStateTime =:= -1 andalso GameOver =:= false andalso RoomType =/= ?Friend_Room_create ),
          Player3;
        _->
          Player
      end;
...
do_quit_logic_friend_ken(Room,Player,QuitType)->
  ...
  #room_ken{id = Id,member = PlayerList,roomstate = RoomState,game_over = GameOver} = Room,
      ...
      case length(TempList) > 0 of
        true->
          ...
          Player3 = case QuitType of
                      ?QUIT_NET_ERROR ->
                        %?IF(IsNotGameStart =:= true,Player#player{room_id = -1,roomtype = -1},Player#player{gamenum = Score});
                        ?INFO(?_U("quit ken:~p"), [1]),
                        change_friend_off_line_state(RoomType,PlayerId,Id),
                        Player#player{gamenum = Score};
                      ?QUIT_BY_SELF->
                        case RoomState =:= ?ROOM_WAIT_READY of
                          true->
                            ken_room_mgr:delete_member(Id,PlayerId);
                          false->
                            change_friend_off_line_state(RoomType,PlayerId,Id)
                        end,
...
do_delete_check_restart(Room,Player,Type,PlayerState,MyPos)->
  ...
  #player{id =PlayerId,roomtype = RoomType,room_id = RoomId} = Player,
  MemberList =  delete_member(RoomType,RoomId,PlayerId,Type,PlayerState,Table_Money),
  %?INFO(?_U("删除房间成员后列表:~p,"), [MemberList]),
  case RoomState of
    ?ROOM_START->
...
do_check_delete_room(Room)->
  ...
  ?IF(IsDelete =:= true,room_friend_mgr:delete(RoomId),ok).
