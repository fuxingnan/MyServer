...
list_test_() ->
    ...
    [
        ...
        ?_assertEqual(ok, list_add(type(), elem_2())),
        ?_assertEqual([elem_2(), elem_1()], get_list(type())),

        % keydelete
        ?_assertEqual({1, dummy}, list_keydelete(type(), 1, 1)),
        ?_assertEqual([elem_2()], get_list(type())),
        ?_assertEqual(false, list_keydelete(type(), 100, 1)),
        ?_assertEqual([elem_2()], get_list(type())),

        % keyfind
        ?_assertEqual(false, list_keyfind(type(), 101, 1)),
        ?_assertEqual(false, list_keyfind(type(), 101, 2)),
        ?_assertEqual({2, dummy}, list_keyfind(type(), 2, 1)),

        % keyupdate
        ?_assertEqual(ok, list_add(type(), elem_1())),
        ?_assertEqual(ok, list_keyupdate(type(), 1, 1, {1, dummy2})),
        ?_assertEqual({1, dummy2}, list_keyfind(type(), 1, 1)),
        ?_assertEqual(ok, list_keyupdate(type(), 1, 1, {3, dummy})),
