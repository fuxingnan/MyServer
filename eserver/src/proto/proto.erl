...
de_packet(<<Section:?SECTION_SIZE, Method:?METHOD_SIZE, Rest/binary>>) ->
    ...
    % 序列号也作为msgid一部分
    %?DEBUG(?_U("收到消息 : ~p ~n"), [Section]),
    MsgId = {Section, Method, 0},
...
en_packet({Section, Method, _Seq}, Record) ->
    % 消息编码成二进制
    Mod = ?PROTO_CONVERT(Section),
    Bin0 = Mod:encode_s2c(Record),
    Bin = ?IF(is_list(Bin0), ?S2B(Bin0), Bin0),

    Res = ?B2S(Bin),
    Length = erlang:length(Res),

   %?DEBUG(?_U("发送消息加密:~w 加密前"), [Res]),
   %?DEBUG(?_U("消息:~w,长度"), [Length]),
    Data1 = world_app:msgEncrypt(Res,Length),
    %?DEBUG(?_U("消息:~w,加密后"), [Data1]),
    %Data1 = Data,
    %Data1 = ?S2B(Data2),
    ...
    Size = iolist_size(Data2) + 2,
    ?ASSERT(Size =< 16#ffffffff - 2),
    %?DEBUG(?_U("消息长度:~w "), [Size]),
    %?DEBUG(?_U("消息id:~w"), [Section]),
    {[<<Size:32, Section:?SECTION_SIZE, Method:?METHOD_SIZE>>, Data2], Size}.
