...
-define(TABLE_RAND_LOSE_MONEY, "table_rank_lose_money").
-define(FULL_FIELDS_Rank_Lose_Money, [
...
record_to_fields(Data=#lose_rank_data{}) ->
    #lose_rank_data{
        id = Id,                               
        cardone = CardOne,                 
        cardtwo = CardTwo,
...
create(Data) ->                                                                                 
    ...
    case ?DB_GAME:insert(?TABLE_RAND_LOSE_MONEY, Vals) of
        {updated, 1} ->                                                                           
            ok;                                                                                   
        {error, _Reason} ->                                                                       
...
load(Id) ->                                                                                       
    case ?DB_GAME:select(?TABLE_RAND_LOSE_MONEY, ?FULL_FIELDS_Rank_Lose_Money, ["id=", db_util:encode(Id)]) of
        {error, _Reason} ->                                                                       
        ...
        {selected, _Fields, []} ->                                                                
            %?DEBUG("Load losedata not found: ~p", [Id]),
            []                                                                                    
    end.                                                                                          
                                                                                                 
%% @doc 更新数据 #rand_win                                                                     
update(Data) ->
    %?INFO(?_U("保存一次输金钱排行榜:~p"), [1]),
    ["id" | Fields] = ?FULL_FIELDS_Rank_Lose_Money,
    [Id | Vals] = record_to_fields(Data),                                                       
    case ?DB_GAME:update(?TABLE_RAND_LOSE_MONEY, Fields, Vals, db_util:where_id(Id)) of
        {updated, 1} ->                                                                           
            ok;                                                                                   
        {updated, 0} ->                                                                          
            ok;                                                                                   
        {error, _Reason} ->                                                                       
            ?ERROR("Update losedata error: ~p ~p", [Id, _Reason]),
            ?C2SERR(?E_DB)                                                                        
    end.                                                                                          
                                                                                                                                                                                                                                                           
                                                                            
decode(Data=#lose_rank_data{ name_list = NameList,image_list = Image_List,money_list = Money_List,pos_list = Pos_List}) ->
    Data#lose_rank_data{
...
encode(Data=#lose_rank_data{ name_list = NameList,image_list = Image_List,money_list = Money_List,pos_list = Pos_List}) ->
    Data#lose_rank_data{
