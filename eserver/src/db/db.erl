...
-export([select/4, select/5, 
        insert/3, insert/4, insert_multi/3,
        update/5, delete/3, delete/2]).
-export([i/0]).
...
prepare(Name, Query, {?MODULE, _POOL} = _This) ->
    mysql:prepare(Name, Query).

%% @doc 取消注册
unprepare(Name, {?MODULE, _POOL} = _This) ->
...
%% @doc 查询请求
select(Table, [H|_] = Fields, Where, Function, {?MODULE, _POOL} = _This)
    when is_list(H), is_list(Where) ->
    Sql = ["select ",
           string:join(Fields, ","),
           " from ", Table,
...
insert(Table, Vals, {?MODULE, _POOL} = _This) ->
    Sql = 
    ["insert into ", Table, 
...
insert(Table, [H|_] = Fields, Vals, {?MODULE, _POOL} = _This) when is_list(H) ->
    Sql = 
...
%% @doc 插入多条数据
insert_multi(_Table, [], {?MODULE, _POOL} = _This) ->
    ok;
insert_multi(Table, ValsList, {?MODULE, _POOL} = _This) when is_list(ValsList) ->
    ValsStrList =
    [begin
        "(" ++ field_join(Vals, ",") ++ ")"
    end || Vals <- ValsList],
...
%% @doc 更新数据
update(Table, [H|_] = Fields, Vals, Where, {?MODULE, _POOL} = _This) when is_list(H) ->
...
%% @doc 删除数据
delete(Table, Where, {?MODULE, _POOL} = _This) when is_list(Where) ->
    Sql =
    ["delete from ", Table, 
    begin
        if 
    ...
    fetch(Sql, _This).
%% @doc 删除所有数据
delete(Table, {?MODULE, _POOL} = _This) ->
    Sql =
    ["delete  from ", Table],
    %?DEBUG("sql is: ~s", [iolist_to_binary(Sql)]),
    fetch(Sql, _This).
