...
-export([fold_by_n/3, run_times/2, unzip_more/1, map_keyfind/3]).
...
-spec unzip_more(list()) -> list().
unzip_more([H | _T] = L) when is_list(H) ->
    unzip_size0(L, length(H));
unzip_more([H|_T] = L) when is_tuple(H) ->
    unzip_size0(L, size(H)).

%% @doc 合并压缩
-spec unzip_size0(list(), non_neg_integer()) -> list().
unzip_size0(L, N) when is_integer(N) ->
    Acc0 = lists:duplicate(N, []),
    lists:foldl(
    fun
        (Ele, Acc) when is_tuple(Ele) ->
            lists:foldl(fun(Index, Acc1) ->
                V = element(Index, Ele),
                NAcc = lists:nth(Index, Acc1) ++ [V],
                replace(Index, Acc1, NAcc)
            end, Acc, lists:seq(1, N));
        (Ele, Acc) when is_list(Ele) ->
                lists:foldl(fun(Index, Acc1) ->
                    V = lists:nth(Index, Ele),
                    NAcc = lists:nth(Index, Acc1) ++ [V],
                    replace(Index, Acc1, NAcc)
                end, Acc, lists:seq(1, N))
    end, Acc0, L).
...
%% @doc map数组的查询
-spec map_keyfind(any(), any(), list()) -> any().
map_keyfind(K, V, DamageAll) ->
    [E] = lists:filter(fun(DamageMap) -> maps:get(K, DamageMap) =:= V end, DamageAll),
    E.
%%------------
%% EUnit TEST
%%------------
