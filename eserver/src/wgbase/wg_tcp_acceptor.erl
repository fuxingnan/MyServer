...
handle_info({inet_async, LSock, Ref, {ok, Sock}},
    ...
    try
        ...
        % handle
        %?INFO(" new connection!", []),
        CallRes = do_callback(Sock, Opts, Callback),
        %?DEBUG("cur count t:~p", [Cur]),
        %?DEBUG("callback res:~p", [CallRes]),
        ?IF(CallRes == ok,accept(State#state{cur = Cur +1}),accept(State#state{cur = Cur}))
    catch 
        {inet_error, Reason} ->
            gen_tcp:close(Sock),
...
handle_info({inet_async, LSock, Ref, {error, Reason}},
    ...
    {stop, normal, State};
handle_info({'DOWN', _MRef, process, Pid, Info}, State = #state{cur = Cur}) ->
    %?DEBUG("client process ~p exit reason:~p", [Pid, Info]),
    %?DEBUG("cur count t:~p", [Cur-1]),
...
do_callback(Sock, Opts, Callback) ->
    ...
    case Result of
        ...
        {error, _Reason} ->
            ...
            no;
        _->
            ?ERROR("spawn client proces error:~p", [no]),
            no
    end.
