...
-define(ROOM, 'room_master').
-define(ROOM_PLAYER, 'room_master_player').
-define(FREE_ROOM_LIST, 'free_room_master_list').
%% @doc 启动
...
handle_call({create, Room_Member}, _From, #state{id_new = IdNew} = State) ->
  ...
  Room = #room
  {
    id = IdNew,
    turn = 0,
    min_money = ?MasterRoom_MinMoney_di,
